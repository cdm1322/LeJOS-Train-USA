...
public class MasterBrick2 implements TimerListener {

	static double seconds = 0;

	static double record;
		
	static Timer timer = new Timer(10, new MasterBrick2());
	...
	public static void main(String[] args) throws Exception {
		...
		BTConnection BTLink = Bluetooth.connect(Bluetooth.getKnownDevice("GRAB2"));
		LCD.drawString("Connecting Loader...", 0, 0);
			
		toLoader2 = BTLink.openDataOutputStream();
		fromLoader2 = BTLink.openDataInputStream();
	...
	public static double distance(int id, int tripEnd, int subtrip) {
		...
		while (id != tripEnd) {
			if (id == 6) {// If at 6, add uncommon distance and loop to 1
				...
				distance = distance + LENGTH_BETWEEN_SENSORS;
				id++;
			} else if (id == 4) {// Identifies if the train will pass the second loader
	...
	public static void Turnout(double direction) {
		...
		// Before starting train system, ensure that both turn-out motors are switched to the inner track
